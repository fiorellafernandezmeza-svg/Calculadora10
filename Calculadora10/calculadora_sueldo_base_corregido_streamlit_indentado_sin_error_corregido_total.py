...
st.warning("""
⚠️ **Importante:** El cálculo mostrado es solo una guía. 
No considera descuentos de 5ta categoría, préstamos, aportes de comedor ni otros. 
...
if tipo_trabajador == "Empleado":
    ...
    porcentaje_descuento = afp_descuento
...
# --- 🧱 Si no es empleado, se ejecuta la parte de obreros (turnos y cálculo por día) ---
    ...
    if turno == "Día":
        ...
        # --- 🔹 Cálculo quincenal ---
            ...
            turno_semana = turno_inicio_pago if 'turno_inicio_pago' in locals() else "Día"
                ...
                if es_feriado(fecha):
                    ...
                    if nombre == "domingo":
                        if turno_semana == "Día":
                            ...
                            pago = neto_noche if 'neto_noche' in locals() else neto_dia
                        feriado_flag = ""
                    else:
                        pago = total_dia if turno_semana == "Día" else (total_noche if 'total_noche' in locals() else total_dia)
                        feriado_flag = ""
...
# --- Turno Rotativo ---
    ...
    if tipo_pago == "Semanal":
        st.markdown("### 📅 Cálculo semanal (mes completo)")
        year = 2025
        mes_num = list(calendar.month_name).index(mes_pago)
        dias_mes = calendar.monthrange(year, mes_num)[1]
        pagos = []
